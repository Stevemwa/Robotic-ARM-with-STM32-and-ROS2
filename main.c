...
//	Motor_Right.Dir = (RxData[0] - 48);
//	Motor_Right.Set_RPM = (((RxData[2] - 48) + ((RxData[1] - 48) * 10))) * 2;
//
//	Motor_Left.Dir = (RxData[3] - 48);
//	Motor_Left.Set_RPM = (((RxData[5] - 48) + ((RxData[4] - 48) * 10))) * 2;

// Parse the data
	char *ptr = RxData;
	while (*ptr != '\0') {
		// Check the identifier and extract following integer angle
		if (*ptr == 'B') {
			Base.SetAngle = atoi(++ptr); // Move to next character, parse number
			Base.Step = (Base.Angle - Base.SetAngle) / 180.0;
		} else if (*ptr == 'S') {
			Shoulder.SetAngle = atoi(++ptr);
			Shoulder.Step = (Shoulder.Angle - Shoulder.SetAngle) / 180.0;
		} else if (*ptr == 'E') {
			Elbow.SetAngle = atoi(++ptr);
			Elbow.Step = (Elbow.Angle - Elbow.SetAngle) / 180.0;
		} else if (*ptr == 'W') {
			Wrist1.SetAngle = atoi(++ptr);
			Wrist1.Step = (Wrist1.Angle - Wrist1.SetAngle) / 180.0;
		} else if (*ptr == 'C') {
			Wrist2.SetAngle = atoi(++ptr);
			Wrist2.Step = (Wrist2.Angle - Wrist2.SetAngle) / 180.0;

		}

		// Move to the next identifier (skip digits)
		while (*ptr >= '0' && *ptr <= '9') {
			ptr++;  // Skip over numbers
		}
	}

	// After parsing, you can now set the servos to the specified angles
	// Example:
	...
	// SetServoAngle(END_EFFECTOR_SERVO, endEffectorAngle);
...
int main(void) {
	...
	MX_USART1_UART_Init();
	/* USER CODE BEGIN 2 */
	HAL_UARTEx_ReceiveToIdle_DMA(&huart1, (uint8_t*) RxData, RxBufferSize);
	ServoInit();
	...
	while (1) {
		...
		if (Base.Angle != Base.SetAngle) {
			Base.MovingAngle += Base.Step;
			Base.Angle = (int) (Base.MovingAngle);
			SetServoAngle(&htim4, TIM_CHANNEL_4, Base.Angle);
			HAL_Delay(1);
		}

		if (Shoulder.Angle != Shoulder.SetAngle) {
			Shoulder.MovingAngle += Shoulder.Step;
			Shoulder.Angle = (int) (Shoulder.MovingAngle);
			SetServoAngle(&htim4, TIM_CHANNEL_3, Shoulder.Angle);
			HAL_Delay(1);
		}

		if (Elbow.Angle != Elbow.SetAngle) {
			Elbow.MovingAngle += Elbow.Step;
			Elbow.Angle = (int) (Elbow.MovingAngle);
			SetServoAngle(&htim4, TIM_CHANNEL_2, Elbow.Angle);
			HAL_Delay(1);
		}
		if (Wrist1.Angle != Wrist1.SetAngle) {
			Wrist1.MovingAngle += Wrist1.Step;
			Wrist1.Angle = (int) (Wrist1.MovingAngle);
			SetServoAngle(&htim4, TIM_CHANNEL_1, Wrist1.Angle);
			HAL_Delay(1);
		}
		if (Wrist2.Angle != Wrist2.SetAngle) {
			Wrist2.MovingAngle += Wrist2.Step;
			Wrist2.Angle = (int) (Wrist2.MovingAngle);
